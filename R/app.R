...
server <- function(input, output, session) {
  ...
  output$boxPlot <- renderPlot({
    ...
    if (pvalue_label == "p.adj.deseq2") {
      ...
      pp <- p + stat_pvalue_manual(stat.test,
        ...
        theme(axis.text = element_text(size = 14),
              ...
              legend.title = element_text(size = 12, face = "bold"),
              plot.title = element_text(size = 15, face = "bold",hjust = 0.5,vjust = 2),
              plot.caption = element_text( size = 12, vjust = 0.1, family= "italic",hjust = 0.1))
    }
    ...
    output$download_boxplot <- downloadHandler(
      ...
      content = function(file) {
        tiff(file,res=300,units="in", width=4, height=4.5)
        print(pp)
        dev.off()
      }
    )
  ...
  output$heatmap_ui <- renderUI({
    ...
    plotOutput("heatmapPlot", width = "500px", height = plot_height)
  })
  ...
  output$heatmapPlot <- renderPlot({
    ...
    output$download_heatmap <- downloadHandler(
      ...
      content = function(file) {
        ...
        tiff(file, res = 300, units = "in", width = 5, height = plot_h)
        heatmap_plot <- Heatmap(top10_cts, name = "Z-score",
                                column_title = paste0("TOP ", num_genes, " gene expression"),
                                col = col_fun,
                                row_names_gp = gpar(fontsize = 10),
    ...
    output$download_matrix <- downloadHandler(
      ...
      content = function(file) {
        ...
        write.csv(final_result, file, row.names = TRUE)
      }
    )
  ...
  output$pcaPlot <- renderPlot({
    ...
    output$download_pca <- downloadHandler(
      ...
      content = function(file) {
        tiff(file,res=300,units="in", width=8.5, height=8)
...
#'
#' Initializes and runs the Shiny application by combining the defined UI and server components.
#' This function launches the app and makes it available for user interaction.
#' @examples
